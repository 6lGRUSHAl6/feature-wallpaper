defmodule Mix.Tasks.Compile.FwRenderer do
  ...
  def run(_args) do
    ...
    with true <- File.exists?(source),
         ...
         {output_text, 0} <- System.cmd(compiler, compile_args(build_dir, source, protocol_code, output), stderr_to_stdout: true) do
  ...
  defp compile_args(build_dir, source, protocol_code, output) do
    ...
    ["-std=c11", "-O2", "-Wall", "-Wextra", "-Wno-unused-parameter", "-I#{build_dir}"] ++
      cflags ++ [source, protocol_code, "-o", output] ++ libs
  end
