defmodule Mix.Tasks.Compile.FwRenderer do
  ...
  def run(_args) do
    ...
    with true <- File.exists?(source),
         ...
         {_, 0} <-
           System.cmd(scanner, ["client-header", xdg_shell_xml, xdg_shell_header],
             stderr_to_stdout: true
           ),
         {_, 0} <-
           System.cmd(scanner, ["private-code", xdg_shell_xml, xdg_shell_code],
             stderr_to_stdout: true
           ),
         {_, 0} <-
           System.cmd(scanner, ["client-header", protocol_xml, protocol_header],
             stderr_to_stdout: true
           ),
         {_, 0} <-
           System.cmd(scanner, ["private-code", protocol_xml, protocol_code],
             stderr_to_stdout: true
           ),
         {output_text, 0} <-
           System.cmd(
             compiler,
             compile_args(build_dir, source, [protocol_code, xdg_shell_code], output),
             stderr_to_stdout: true
           ) do
      ...
      false ->
        {:error, ["missing build input for fw renderer"]}
